...
export const signUpSelector = selector({
    ...
    get: async ({get}) => {
        ...
        const res = await axios.post('http://localhost:3000/api/v1/user/signup', {signUpAtom})
        return res.data;
    }
})
...
export const userDetailsSelectorFamily = selector({
    ...
    get: async ({get}) => {
        ...
        const res = await axios.get(`http://localhost:3000/api/v1/user/bulk?filter=${user}`);
